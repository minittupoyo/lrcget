<template>
  ...
  >
    ...
    <div v-else-if="props.lyrics.plainLyrics" class="relative grow text-center text-brave-50 whitespace-pre h-full overflow-hidden">
      <div class="grow p-4 h-full overflow-y-auto">{{ props.lyrics.plainLyrics }}</div>
    </div>
  </BaseModal>
</template>
...
const { status, duration, progress, playTrack, pause, resume, stop, seek } = usePlayer()
...
const fullViewTransform = computed(() => {
  if (!currentLineElementOffset.value) return `translateY(calc(50% - 2.5em))`
...
watch(progress, (newProgress) => {
  if (!runner.value || !props.lyrics) return
  runner.value.timeUpdate(newProgress)
  let resultCurrentIndex = runner.value.curIndex()
  if (resultCurrentIndex === null) resultCurrentIndex = -1
