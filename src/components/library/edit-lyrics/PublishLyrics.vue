  ...
  >
    <template #default>
            ...
            <tbody class="text-xs">
              <tr v-for="(problem, index) in lintResult" :key="index">
                ...
                <td class="p-1 text-center">
                  <span v-if="problem.severity === 'error'" class="bg-red-200 text-red-800 dark:bg-red-900 dark:text-red-100 font-bold text-xs px-1 py-0.5 rounded">{{ $t('publish.error') }}</span>
                </td>
      ...
      <div v-else class="flex flex-col items-center">
        <div v-if="!isPublishing" class="mb-4">
          {{ $t('publish.confirmSynced', { track: `${title} - ${artistName}` }) }}
        </div>
        <div v-else class="mb-4">
          {{ $t('publish.publishingSynced', { track: `${title} - ${artistName}` }) }}
        </div>
        ...
        <table v-if="isPublishing" class="text-xs table-auto font-mono uppercase">
          <tbody>
            ...
            <tr>
              <td class="px-2 py-1">{{ $t('publish.publishLyrics') }}</td>
    ...
    <template #footer>
      <div v-if="lintResult.length" class="flex gap-2 justify-center w-full">
        <button class="button button-primary px-8 py-2 rounded-full" @click="close">{{ $t('publish.close') }}</button>
      </div>

      <div v-else-if="!isPublishing" class="flex gap-2 justify-center w-full">
        <button class="button button-primary px-8 py-2 rounded-full" @click="publishLyrics">{{ $t('publish.publishNow') }}</button>
...
const publishLyrics = async () => {
  ...
  try {
    ...
    toast.success(t('publish.successSynced'))
  } catch (error) {
    isError.value = true
    console.error(error)
    toast.error(error)
